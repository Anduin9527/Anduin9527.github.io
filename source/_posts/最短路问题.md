...
+ 利用上述条件，Floyd算法可以快速判断图中是否存在**负圈**，也就是判断$diag(d)$是否存在小于0的值
...
+ Bellman算法从$s$出发，如果抵达一个负环，松弛操作会无限进行下去，但是由于最短路边数存在上限$n-1$，所以也可以快速判断**负圈**
+ 上述结论其实不大严谨
...
根据前面的思想可以知道，计算和调整结点$u$到结点$s$的最短距离后，如果紧接着调整$u$的邻居结点，势必会触发新的**松弛操作**，而Bellman算法，则在尝试大量不相关结点的边松弛操作，这显然是低效的。
...
比较简单，用一个整形数组path，在更新距离的时候，记录点是如何转移的即可
